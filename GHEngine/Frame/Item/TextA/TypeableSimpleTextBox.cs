...
namespace GHEngine.Frame.Item.Text;
...
public class TypeableSimpleTextBox : SimpleTextBox
{
    ...
    public void SetCursorPosition(Vector2 pixelLocation)
    {
        ...
        if (!CharWasFound)
        {
            LinePixelPosition.SetValue(LinePixelPosition.Current);
        }
    ...
    {
        if (!IsLocationOverBox(UserInput._virtualMousePosition.Current))
        {
            IsFocused = false;
            return;
        }
        ...
        IsFocused = true;
        SetCursorPosition(UserInput._virtualMousePosition.Current);
    }
    ...
    public override void Draw()
    {
        ...
        GameFrameManager.s_spriteBatch.Draw(DisplayOld.SinglePixel,
            DisplayOld.ToRealPosition(_cursorPositionVisual),
            null,
            CursorColor,
            0f,
            Vector2.Zero,
            DisplayOld.ToRealScale(new Vector2(2.5f, Font.LineSpacing)),
            SpriteEffects.None,
            IRenderableItem.DEFAULT_LAYER_DEPTH);
    }
}