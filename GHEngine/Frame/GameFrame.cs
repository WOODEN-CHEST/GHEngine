...
public class GameFrame : IGameFrame
{
    ...
    public virtual void Draw(IDrawInfo info, RenderTarget2D layerPixelBuffer, RenderTarget2D framePixelBuffer)
    {
        ...
        foreach (ILayer FrameLayer in _layers)
        {
            ...
            DisplayOld.GraphicsManager.GraphicsDevice.SetRenderTarget(layerPixelBuffer);
            DisplayOld.GraphicsManager.GraphicsDevice.Clear(Color.Transparent);
            FrameLayer.Draw(info);

            DisplayOld.GraphicsManager.GraphicsDevice.SetRenderTarget(framePixelBuffer);
