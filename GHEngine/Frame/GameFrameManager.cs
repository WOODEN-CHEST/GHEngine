...
public static class GameFrameManager
{
    ...
    public static void DrawFrames(IProgramTime time)
    {
        ...
        DisplayOld.GraphicsManager.GraphicsDevice.SetRenderTarget(s_framePixelBuffer);
        DisplayOld.GraphicsManager.GraphicsDevice.Clear(Color.Black);
        ActiveFrame.Draw(s_drawInfo, s_layerPixelBuffer, s_framePixelBuffer);

        DisplayOld.GraphicsManager.GraphicsDevice.SetRenderTarget(null);
    ...
    private static void CreateLayerPixelBuffer()
    {
        ...
        s_layerPixelBuffer = new(DisplayOld.GraphicsManager.GraphicsDevice,
            (int)DisplayOld.WindowSize.X, (int)DisplayOld.WindowSize.Y, false, SurfaceFormat.Color, 
            DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
        s_framePixelBuffer = new(DisplayOld.GraphicsManager.GraphicsDevice,
            (int)DisplayOld.WindowSize.X, (int)DisplayOld.WindowSize.Y, false, SurfaceFormat.Color,
            DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
    }
